...
function del() {
if (currentOp.innerHTML.length > 0) {    currentOp.innerHTML=currentOp.innerHTML.slice(0, -1);
}
else {
...
function zerofunc(num) {
    if (+previousOp.innerHTML >1e-10)
    previousOp.innerHTML+=num.innerHTML;

}
function zerofunc2(num) {
    if (+currentOp.innerHTML >1e-10)
    currentOp.innerHTML+=num.innerHTML;
...
function signfunc(sign, func){
    if (previousOp.innerHTML.length < 1){changefunc2()}
    else{
    changefunc()
